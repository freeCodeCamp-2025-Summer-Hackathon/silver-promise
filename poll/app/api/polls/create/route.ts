...
export async function POST(request: Request) {
    const { title, description, options, question, type } = await request.json();
    ...
    if (!question || !options || options.length < 2) {
        return Response.json({ success: false, message: "Invalid poll data" }, { status: 400 });
    }
    ...
    const newPoll = await PollRepository.createPoll({
        ...
        description,
        question,
        options: options.map((option: { text: string }) => ({ text: option.text, votes: 0 })),
        type: typeR
    }, user.userId);

    if (!newPoll) {
        return Response.json({ success: false, message: "Failed to create poll" }, { status: 500 });
    }
    ...
    return Response.json({ success: true, poll: { ...nPoll } }, { status: 201 });
}
