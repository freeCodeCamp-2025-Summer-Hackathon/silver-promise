...
export const CreatePoll = () => {
  ...
  const [singlechoiceOptions, setSingleChoiceOptions] = useState<PollOption[]>([
    ...
    if (editingOptionId) {
      ...
      if (valueToSet) {
        setSingleChoiceOptions((prev) =>
          prev.map((opt) =>
            opt.id === editingOptionId ? { ...opt, label: valueToSet, value: valueToSet } : opt
          )
        );
      }
    }
  ...
  return (
    <section className=" flex flex-col max-h-screen m-4 p-6  bg-[#f7f7f7] rounded-2xl ">
      <section className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-6">
        <div className=" bg-white min-h-screen rounded-xl p-6">
          <form>
            ...
            <fieldset className="mt-6 p-6 border rounded-xl border-dark-gray">
                ...
                <div>
                  {pollType === "single-choice" && (
                    <div className="text-sm text-dark-gray">
                        ...
                        <div key={option.id} className="flex items-center mb-2">
                          ...
                          {editingOptionId === option.id ? (
                            <div ref={editRef} className="flex items-center gap-2">
                              ...
                              <button
                                type="button"
                                onClick={handleUpdateAndExitEditMode}
                      ...
                      <button
                        type="button"
                        onClick={handleAddSinglChoiceOption}
                  ...
                  {pollType === "multiple-choice" && (
                    <p className="text-sm text-dark-gray">You selected Multiple Choice Poll Type</p>
                  )}
                  {pollType === "open-ended" && (
                    <p className="text-sm text-dark-gray">You selected Open Ended Poll Type</p>
                  )}
                </div>
              </div>
            </fieldset>
        ...
        <div className="bg-white min-h-screen rounded-xl p-6">This is the poll section</div>
      </section>
    </section>
  );
};
